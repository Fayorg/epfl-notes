...
\end{definition}
\begin{proof}
    Let's prove that $z z^{-1} = 1$. We have:
    \[
...
\begin{eg}
    We can prove that $\mathbb{C}$ is not an ordered field.  \\
    Indeed, if we assume that $i > 0$, then $i^2 = -1 > 0$ which is a contradiction. If we assume that $i < 0$, then $-i > 0$ and $(-i)^2 = -1 > 0$ which is also a contradiction. Thus, $\mathbb{C}$ is not an ordered field.
\end{eg}

\section{Forms of Complex Numbers}
Complex numbers can be represented in 3 different forms, namely: algebraic form (being the most common), trigonometric form and exponential form.
\begin{definition}[Algebraic Form]
    The algebraic form of a complex number $z$ is denoted as:
    \[
        z = a + bi = \text{Re}(z) + \text{Im}(z)i
    \]
\end{definition}
% Complex numbers can be represented in 3 different forms. But first, let's defined the modulus and the argument of a complex number.

\begin{definition}[Real and Imaginary Parts]
    Let $z = a + bi \in \mathbb{C}$. The real part of $z$ is defined as $\text{Re}(z) = a$ and the imaginary part of $z$ is defined as $\text{Im}(z) = b$.
\end{definition}
...
\begin{definition}[Argument]
    The argument of a complex number $z \neq 0$ is defined as the angle $\theta$ such that:
    \[
        \cos(\theta) = \frac{\text{Re}(z)}{|z|} \quad \text{and} \quad \sin(\theta) = \frac{\text{Im}(z)}{|z|}
    \]
    The argument is denoted as $\text{arg}(z)$ and is defined up to an additive factor of $2k\pi$, $k \in \mathbb{Z}$.
\end{definition}
...
\begin{definition}[Trigonometric Form]
    ...
    \begin{center}
        \begin{tikzpicture}[scale=.85]
            ...
            \draw[thick, primary] (0,0) -- (2,1) node[above right] {$z = a + bi$};
            \draw[dashed, secondary] (2,1) -- (2,0);
            \draw[dashed, secondary] (2,1) -- (0,1);
            \draw[->, secondary] (0.5,0) arc[start angle=0,end angle=26.57,radius=0.5];
            \node[secondary] at (1,0.25) {$\theta$};
            \node[primary] at (1,.75) {$r$};
            \node[secondary] at (2,-0.5) {$a = r \cos(\theta)$};
            \node[secondary] at (-1.4,1) {$r \sin(\theta) = b$};
        \end{tikzpicture}
    \end{center}
    and:
    \[
        \text{Re}(z) = a = r \cos(\theta) \quad \text{and} \quad \text{Im}(z) = b = r \sin(\theta)
    \]
\end{definition}
The modulus of $z$ is given by:
\[
    |z| = r = \sqrt{r^2 \cos^2(\theta) + r^2 \sin^2(\theta)}
\]
The argument of $z$ ($z \neq 0$) is given by:
\[
    r \neq 0 \quad \implies \quad \sin(\theta) = \frac{\text{Im}(z)}{r} \ \text{and} \ \cos(\theta) = \frac{\text{Re}(z)}{r}
\]
...
\begin{proof}
    ...
    \[
        y_1 = y_2 + 2k\pi, \ k \in \mathbb{Z} \quad \implies \quad e^{i y_1} = e^{i y_2 + 2k\pi} = e^{i y_2}(\cos(2k\pi) + i \sin(2k\pi)) = e^{i y_2}
    \]
\end{proof}
...
\subsection{Summary of the Forms of Complex Numbers}
...
\begin{proof}
    ...
    \]
    Now, let's assume it's true for $n = k$, i.e., $z^k = r^k e^{i k \theta}$ and show that it holds for $n = k + 1$. We have:
    \[
...
\section{Conjugate of Complex Numbers}
...
\[
    \cos(\phi) = \frac{e^{i\phi} + e^{-i\phi}}{2} \quad \text{and} \quad \sin(\phi) = \frac{e^{i\phi} - e^{-i\phi}}{2i}
\]
...
\begin{definition}[n-th Roots of Complex Numbers]
...
\begin{proof}
    ...
    \[
        \rho^n = r \quad \implies \quad \rho = r^{\frac{1}{n}}
    \]
    and
    \[
        n \phi = \theta + 2k\pi \quad \implies \quad \phi = \frac{\theta + 2k\pi}{n},\quad k \in \mathbb{Z}
    \]
...
\begin{eg}
    ...
    \[        z_0 = r^{\frac{1}{2}} e^{i \frac{\theta}{2}} \quad \text{and} \quad z_1 = r^{\frac{1}{2}} e^{i \left(\frac{\theta}{2} + \pi\right)} = -r^{\frac{1}{2}} e^{i \frac{\theta}{2}} = -z_0
    \]
    More explicitly:
    \[
        (z_1)^2 = (z_2)^2 = w
...
\begin{eg}
    ...
    \[
        z = r e^{i \theta} \quad \implies \quad \bar{z} = r e^{-i \theta}
    \]
    Thus:
    \[
    ...
    \]
    and
    \[        6 \theta = -\theta + 2k\pi \quad \implies \quad 7 \theta = 2k\pi \quad \implies \quad \theta = \frac{2k\pi}{7}, \quad k \in \mathbb{Z}
    \]
...
\end{proof}\noindent\ignorespaces % special fix for here idk what happened bc it works on all the other sections wtf
...
\begin{eg}
    ...
    \[        z = r e^{i \theta} \quad \implies \quad |z| = r \quad \implies \quad \frac{|z|}{z} = \frac{r}{r e^{i \theta}} = e^{-i \theta}
    \]
    Thus:
    \[        (e^{-i \theta})^4 = e^{-4i \theta} = \frac{1}{\sqrt{2}}(1 + i) = e^{i \frac{\pi}{4}}
    \]
    By comparing the arguments, we get:
    \[        -4 \theta = \frac{\pi}{4} + 2k\pi \quad \implies \quad \theta = -\frac{\pi}{16} - \frac{k\pi}{2}, \quad k = 0,1,2,3
    \]
    Thus, the solutions are:
    \[        \theta \in \left\{ -\frac{\pi}{16} - \frac{k\pi}{2} : k = 0,1,2,3 \right\}
    \]
