...
Notes:
\begin{itemize}[itemsep=1pt,label=$\circ$]
    \item Matrix multiplication is generally not commutative, i.e. \( AB \neq BA \) in most cases.
    \item Generally, matrix multiplication is not cancellative, i.e. from \( AB = AC \), we cannot generally conclude that \( B = C \).
