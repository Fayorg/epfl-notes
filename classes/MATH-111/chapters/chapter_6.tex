...
\end{theorem}
\begin{proof}
    ($\Rightarrow$) Let $\vec{u} \in W^{\perp}$. Then, for any $\vec{w} \in W$, we have $\vec{u} \cdot \vec{w} = 0$. In particular, for each basis vector $\vec{w_i}$, we have $\vec{u} \cdot \vec{w_i} = 0$. \\
    ($\Leftarrow$) Let $\vec{u}$ be such that $\vec{u} \cdot \vec{w_i} = 0$ for all $i = 1, 2, \ldots, k$. Any vector $\vec{w} \in W$ can be expressed as a linear combination of the basis vectors:
    \[
...
\begin{proof}
    ...
    (1) $\implies$ (2): If the columns of $A$ are linearly independent, then $\ker(A) = \{\vec{0}\}$. This implies that $A^T A$ is positive definite and thus invertible. \\
    (2) $\implies$ (3): If $A^T A$ is invertible, then for any $\vec{b} \in \mathbb{R}^m$, the normal equations $A^T A \vec{\hat{x}} = A^T \vec{b}$ have a unique solution given by:
    \[
        \vec{\hat{x}} = (A^T A)^{-1} A^T \vec{b}
    \]
    Thus, the equation $A \vec{x} = \vec{b}$ has a unique least squares solution. \\
    (3) $\implies$ (1): If the equation $A \vec{x} = \vec{b}$ has a unique least squares solution for every $\vec{b} \in \mathbb{R}^m$, then $\ker(A)$ must be trivial. If there were a non-zero vector $\vec{v} \in \ker(A)$, then for any least squares solution $\vec{\hat{x}}$, the vector $\vec{\hat{x}} + \vec{v}$ would also be a least squares solution, contradicting the uniqueness. Therefore, the columns of $A$ are linearly independent. \\
